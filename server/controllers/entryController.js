    ...
    try {
      ...
      if (status === 'accepted') {
        try {
          ...
          try {
            if (tournament.autoGenerateOnMinReached && tournament.meta.teamsCount >= (tournament.minTeams || 2) && ['upcoming','open'].includes(tournament.status)) {
              ...
              console.log(`🎯 Auto-generating fixtures for tournament ${tournament.name} with ${teamIds.length} teams`);
              
              await FixtureService.generateRoundRobin(tournamentId, teamIds, { 
                scheduleStart: tournament.startDate || new Date(), 
                intervalMinutes: 60 
              });
              ...
              console.log(`✅ Tournament ${tournament.name} status updated to 'scheduled' with fixtures generated`);
            }
          } catch (e) { 
  ...
  await tournament.save();
    ...
    try {
      if (tournament.autoGenerateOnMinReached && tournament.meta.teamsCount >= (tournament.minTeams || 2) && ['upcoming','open'].includes(tournament.status)) {
        ...
        console.log(`🎯 Auto-generating fixtures after approval for tournament ${tournament.name} with ${teamIds.length} teams`);
        ...
        console.log(`✅ Tournament ${tournament.name} fixtures generated and status updated to 'scheduled'`);
      }
    } catch (e) {
      console.warn('Auto-generate fixtures failed:', e && e.message ? e.message : e);
    }
